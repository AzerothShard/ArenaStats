...
(function () {
  ...
  app.controller("TeamDetailsController", function($rootScope, $stateParams, $scope, $http) {
    ...
    /* onchange $rootScope.season reload team statistics */
    ...
    $scope.loadTeam = function() {
      ...
      $http.get( app.api + "arena_team/id/" + $stateParams.id + "?season=" + $rootScope.season )
        ...
        if (data.length > 0) {
          ...
          $http.get( app.api + "arena_team_member/" + $stateParams.id + "?season=" + $rootScope.season )
            .success(function(data, status, header, config) {
